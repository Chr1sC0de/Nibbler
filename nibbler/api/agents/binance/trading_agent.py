...
class TradingAgentBase(object):
    ...
    def post_sell_reduce_order(self, *, price, quantity, **kwargs):
        result = self.requester.post_order(
            symbol=self.symbol,
            side=bf.model.OrderSide.SELL,
            ordertype=bf.model.OrderType.TAKE_PROFIT,
            timeInForce="GTX",
            reduceOnly=True,
            quantity=quantity,
            price=price,
    ...
    def sell_stop_limit_order(self, *, stopPrice, price, quantity, **kwargs):
        result = self.requester.post_order(
            symbol      = self.symbol,
            side        = bf.model.OrderSide.SELL,
            ordertype   = bf.model.OrderType.STOP,
            timeInForce = "GTX",
            quantity    = quantity,
            price       = price,
            stopPrice   = stopPrice,
    ...
    @is_successful
    def buy_stop_limit_order(self, *, stopPrice, price, quantity, **kwargs):
        result = self.requester.post_order(
            symbol      = self.symbol,
            side        = bf.model.OrderSide.BUY,
            ordertype   = bf.model.OrderType.STOP,
            timeInForce = bf.model.TimeInForce.INVALID,
            quantity    = quantity,
            price       = price,
    ...
    @is_successful
    def buy_stop_reduce_limit_order(self, *, stopPrice, price, quantity, **kwargs):
        result = self.buy_stop_limit_order(
            stopPrice=stopPrice, quantity=quantity, reduceOnly=True, **kwargs)
        return result

    @is_successful
    def buy_stop_market_order(self, *, stopPrice, quantity, **kwargs):
        result = self.requester.post_order(
            symbol      = self.symbol,
            side        = bf.model.OrderSide.BUY,
            ordertype   = bf.model.OrderType.STOP_MARKET,
    ...
    @is_successful
    def buy_stop_reduce_market_order(self, *, stopPrice, quantity, **kwargs):
        result = self.buy_stop_market_order(
            stopPrice=stopPrice, quantity=quantity, reduceOnly=True, **kwargs)
        return result
